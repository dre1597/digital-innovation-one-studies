...
import org.example.reactivebingo.domain.dto.enumeration.RoundSortBy;
...
public class RoundRepositoryImpl {
  ...
  public Flux<RoundDocument> findOnDemand(final PageRequestDTO<RoundSortBy> request) {
  ...
  public Mono<Long> count(final PageRequestDTO<RoundSortBy> request) {
    return Mono.just(new Query())
        .flatMap(query -> buildWhere(query, request.sentence()))
