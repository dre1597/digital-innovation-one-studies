...
SELECT Ano, COUNT(1) Quantidade FROM Filmes GROUP BY Ano ORDER BY Duracao DESC;
...
SELECT f.Nome, a.PrimeiroNome, a.UltimoNome, ef.Papel FROM Filmes f INNER JOIN ElencoFilme ef ON f.Id = ef.IdFilme INNER JOIN Atores a ON f.Id = ef.IdFilme INNER JOIN Atores a ON ef.IdAtor = a.Id;
